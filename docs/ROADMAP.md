...
- **Libro de Boletas Electrónicas (envío 4).** Aplica: el instructivo lo pide sin condición —el paréntesis sólo *agrega* algo para emisores de ticket de espectáculo—, así que no hacía falta preguntarlo. Se implementó con el XSD oficial `LibroBOLETA_v10.xsd` (bajado del `schema_libro_bol.zip` del SII): [`LibroBoletaXmlGenerator`](../backend/src/main/java/cl/nexosoftware/factura/tributario/LibroBoletaXmlGenerator.java) + [`LibroBoletaService`](../backend/src/main/java/cl/nexosoftware/factura/libro/LibroBoletaService.java), `GET /api/empresas/{id}/libro-boletas/xml-firmado?periodo=&folioNotificacion=`. No se envía: es adjunto del correo.
...
**Falta:**
- **El sitio web público de consulta de la boleta.** El SII lo exige señalado en la representación impresa y **disponible en la web**, y avisa que lo verifica al autorizar por resolución. El PDF no imprime ninguna URL y el sitio no existe. Es requisito de aprobación. No bloquea las 24 horas del set: el PDF se renderiza a pedido desde el XML almacenado, así que se puede emitir primero y generar los PDF después, ya con la URL.
- **Las 10 muestras en PDF** del envío 5: son emisiones reales (10 folios más, aparte de los 5 del set) y deben tener relación con el giro. Conviene emitirlas **otro día** que el set: el RCOF agrega por día, y si van juntas el RCOF «asociado al set» declararía 15 folios en vez de 5.
