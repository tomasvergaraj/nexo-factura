...
export function FacturaPreview({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      className={`overflow-hidden rounded-[14px] border border-line bg-white shadow-[0_18px_50px_-20px_rgba(11,27,43,0.28)] ${className}`}
    >
      {/* pliegue de papel */}
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="font-display text-base font-bold text-ink">Nexo Software SpA</div>
            <div className="mt-0.5 text-xs text-slate">Desarrollo de software · Quillota</div>
            <div className="text-xs text-slate-soft">{formatRut("765432109")}</div>
          </div>
          <div className="rounded-md border-2 border-danger px-3 py-2 text-center">
            <div className="text-[10px] font-semibold uppercase tracking-wide text-danger">R.U.T. 76.543.210-9</div>
            <div className="text-[11px] font-semibold text-danger">Factura Electrónica</div>
            <div className="font-display text-sm font-bold text-danger tnum">N° 142</div>
          </div>
        </div>

        {/* receptor */}
        <div className="mt-4 rounded-lg bg-mist px-3 py-2 text-xs">
          <span className="text-slate-soft">Señor(es): </span>
          <span className="font-medium text-ink">Constructora Andes SpA</span>
          <span className="text-slate-soft"> · {formatRut("782223334")}</span>
        </div>

        {/* detalle */}
        <div className="mt-4">
          <div className="grid grid-cols-[1fr_auto] gap-2 border-b border-line pb-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-soft">
            <span>Detalle</span>
            <span className="text-right">Importe</span>
          </div>
          {lineas.map((l) => (
            <div key={l.n} className="grid grid-cols-[1fr_auto] gap-2 border-b border-line/70 py-2 text-xs">
              <span className="text-ink">
                {l.n} <span className="text-slate-soft">×{l.c}</span>
              </span>
        ...
        <div className="mt-4 flex items-end justify-between gap-6">
          <div className="flex-1">
            <div className="grid h-16 place-items-center rounded-md border border-dashed border-slate-soft/60 bg-mist/60 text-center">
          ...
          <div className="w-40 space-y-1 text-xs">
            ...
            <div className="mt-1 flex items-center justify-between border-t border-line pt-1.5">
              ...
              <span className="font-display text-sm font-bold text-cobalt tnum">{formatCLP(total)}</span>
            </div>
          </div>
        </div>
      </div>
